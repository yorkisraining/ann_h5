    ...
    <div class="confirm_order"  :style="{minHeight: minH + 'px'}">
        ...
        <div class="card_item">
            ...
            <div class="msg_list">
                ...
                <div class="msg_list_item" v-if="isShowAppointTime">
                    <div>预约时间</div>
        ...
        <div class="appoint_btn_box" v-if="type == 1">
            <div :class="['appoint_btn', {cur: curBtn}]" @click="clickCur">立即取餐</div>
            <div :class="['appoint_btn', {cur: !curBtn}]" @click="clickAppoint">预约取餐 <span v-show="!curBtn">{{appointTime}}</span></div>
        </div>
        <div :class="['type_btn', {'default_btn': type == 0 || type == 3}]" @click="pay">{{type | filterTypeBtn}}</div>
...
export default {
    ...
    created() {
        ...
        ajaxGet(`${apiUrl.baseURL}app/goodOrder/info/${this.orderId}`, {}, res => {
            ...
            this.isShowAppointTime = res.bookTime != null ? true : false;
                ...
                if (!this.isClickPay) {
                    ...
                    if (this.curBtn) {
                        ...
                        ajaxGet(`${apiUrl.baseURL}app/goodOrder/book/1/${this.orderId}`, {}, res => {
                            ...
                            this.$router.replace(`/choosePayFn?module=2&id=${this.orderId}`);
                        }, res => {
                            this.isClickPay = false;
                        })
                    } else {
